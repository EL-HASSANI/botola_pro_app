...
st.set_page_config(page_title='Botola AI Insights',
                   page_icon='🥅',
                   layout='wide',
...
position_mapping=d.position_mapping
stats_mapping=d.stats_mapping
spaces = '&nbsp;'
pos = {'GK':'Goalkeepers', 'DF':'Defenders', 'MID':'Midfielders', 'FWD':'Forwards'}

exclude_columns=d.exclude_columns
...
def display_stat(label, value, df, stat):
    ...
    st.markdown(f"""
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
            <span style="color: #030202;">{label}</span>
            <span style="color: #030202;">{value + " min" if label == 'Scoring Frequency' else value}</span>
        </div>
...
def player_details(df, player_data):
    heatmap_df = pd.read_csv('C:/Users/hp/Desktop/BotolaFoot/Botola-Scout/dataset botola foot/Botola Players HeatMaps.csv')
    shotmap_df = pd.read_csv('C:/Users/hp/Desktop/BotolaFoot/Botola-Scout/dataset botola foot/Botola Players ShotMaps.csv')
    spaces = '&nbsp;'
    ...
    with col1:
        lottie_animation = p.load_lottieurl("https://lottie.host/a79be04b-d18c-4f2b-8341-9bd28d0ab5d5/ke10ceU8Pg.json")
    ...
    with col1:
        ...
        p.display_player_card(player_data)
    with col2:
    ...
    with col1:
        ...
        if position_group == 'GK':
            ...
            gk_df = pd.read_csv('C:/Users/hp/Desktop/BotolaFoot/Botola-Scout/dataset botola foot/GK Attributes.csv')
...
def filter_page(df):
    ...
    col1, col2 = st.columns((1, 3))  # Adjust the ratio of the columns as needed
    
    with col1:
        st.image(botola_png, width=300)
    ...
    with col2:
        st.markdown("""
            ...
            .colored-line {
                height: 3px;
                width: 100%;  /* Adjust the width here if necessary */
                max-width: 400px;
        ...
        st.markdown("""
                    <div class="container">
                        ...
                        <p class="big-font">
                            Scout <span style="color: #8c4bb8;">Wise</span>, Teams <span style="color: #8c4bb8;">Rise</span>!
                        </p>
                    </div>
                """, unsafe_allow_html=True)
        ...
        #st.markdown('<p class="big-font">Scout <span style="color: #8c4bb8;">Wise</span>, Teams <span style="color: #8c4bb8;">Rise</span>!</p></div>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.markdown("<h1><span style='color:#FFFFFF; font-size:20px; font-family:cursive'>Filters </span></h1>", unsafe_allow_html=True)
    
        st.markdown("""
            <h2 style='color: rgb(15, 15, 15); font-size: 18px; font-family: cursive;'>
        ...
        p = sac.checkbox(
            items=positions,
            label='', index=[2], align='center', size='md', color='rgb(69, 68, 68)', return_index=True
        )
        ...
        f = sac.checkbox(
            items=foots,
            label='', index=[0,1,2], align='center', size='md', radius='lg', color='rgb(69, 68, 68)', return_index=True
        )
        ...
        with col2:
            search = st.button("Search Players")
    ...
    if not search:
        ...
        # Chemin du dossier contenant les images des logos des équipes
        image_folder = 'C:/Users/hp/Desktop/BotolaFoot/Botola-Scout/logo'

        # Charger les images et les chemins d'accès complets
        images, image_paths = load_images_from_folder(image_folder)

        # CSS pour ajuster la taille et créer un effet de défilement
        st.markdown(d.style_img, unsafe_allow_html=True)

        # Créer un conteneur d'images qui défile
        image_html = '<div class="image-container"><div class="image-track">'

        # Ajouter chaque image sous forme base64 dans le HTML
        for img in images:
            img_base64 = image_to_base64(img)  # Conversion en base64
            # Insertion des images dans la balise <img> avec encodage base64
            image_html += f'<img src="data:image/png;base64,{img_base64}" alt="Logo">'

        image_html += '</div></div>'

        # Afficher le HTML avec les images qui défilent
...
def image_to_base64(img):
    buffered = BytesIO()
    img.save(buffered, format="PNG")  # Assuming PNG format, adjust if needed
    return base64.b64encode(buffered.getvalue()).decode()


def main():

    df = load_data('C:/Users/hp/Desktop/BotolaFoot/Botola-Scout/dataset botola foot/Final_Players_Dataset.csv')
